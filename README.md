...
| Draft Order                | basketball-reference.com               | 1,258   | 1997-2024    | Draft order for players. Includes allocation drafts assigned as draft position 1.                                                                    |
## Installation
...
Results will appear in `results/` folder. All obtained data from will be stored in `data/`
...
Analysis was performed using Decision Tree and Random Forest Modeling. 38 matching NCAA features were normalized using 
...
*Due to data source limitations, the number of games a player was a starter was not included in the NCAA feature set.*
...
explains 24.2% of the variation in WNBA performance outcomes, not adjusted for position (*R2*=.242). The mean square error
showed regression predictions have an average error of one performance tier (*MSE* = 1.146).
