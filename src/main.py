...
def main(args):
    ...
    if args.load:
        ...
        print(f'Total number of players included in sample: {len(includedAthletes)}.')

        #WNBA featuere variables
        #WNBA game level calculations
        WNBAData['field_goal_percentage'] = WNBAData['field_goal_percentage'].fillna(pr.compute_ratios(WNBAData,'field_goals_made','field_goals_attempted'))
        WNBAData['three_point_field_goal_percentage'] = WNBAData['three_point_field_goal_percentage'].fillna(pr.compute_ratios(WNBAData, 'three_point_field_goals_made', 'three_point_field_goals_attempted'))
        WNBAData['free_throw_percentage'] =  WNBAData['free_throw_percentage'].fillna(pr.compute_ratios(WNBAData, 'free_throws_made', 'free_throws_attempted'))
        ...
        WNBAData['three_point_attempt_rate'] = WNBAData['three_point_attempt_rate'].fillna(pr.compute_ratios(WNBAData,'three_point_field_goals_attempted','field_goals_attempted'))
        WNBAData['free_throw_attempt_rate'] = WNBAData['free_throw_attempt_rate'].fillna(pr.compute_ratios(WNBAData,'free_throws_attempted','field_goals_attempted'))
        ...
        # add draft data to WNBA df and clear NA
        finalWNBAFeatures = finalWNBAFeatures.merge(draftData[['athlete_display_name', 'draft_pick']], on='athlete_display_name',how='left')
        finalWNBAFeatures['draft_pick'] = finalWNBAFeatures['draft_pick'].fillna(999)
        print('Draft position added to features data.')

        #save file
        finalWNBAFeatures.to_parquet(config.dataDirCleaned / config.finalWNBAFeaturesFile) #store features for visualization
        print(f'WNBA features generated and saved to {config.dataDirCleaned}')
        ...
        finalNCAAFeatures['three_point_field_goal_percentage'] = pr.compute_ratios(finalNCAAFeatures, 'three_point_field_goals_made', 'three_point_field_goals_attempted')
        finalNCAAFeatures['free_throw_percentage'] = pr.compute_ratios(finalNCAAFeatures, 'free_throws_made', 'free_throws_attempted')
        ...
        finalNCAAFeatures['three_point_attempt_rate'] = pr.compute_ratios(finalNCAAFeatures, 'three_point_field_goals_attempted', 'field_goals_attempted')
        finalNCAAFeatures['free_throw_attempt_rate'] = pr.compute_ratios(finalNCAAFeatures, 'free_throws_attempted', 'field_goals_attempted')
    ...
    if args.evaluation:
        ...
        print(f'Decision tree performance for Centers. Accuracy: {accuracyCenterDT}. Report: {reportCenterDT}. Confusion Matrix: {cmCenterDT}')
        ...
        print(f'Ensemble performance for Centers. Accuracy: {accuracyCenterEN}. Report: {reportCenterEN}. Confusion Matrix: {cmCenterEN}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load",action="store_true",help="Load and clean data from data sources.")
    parser.add_argument("--evaluation",action="store_true",help="Train and evaluate model.")
    parser.add_argument("--dataURL",type=str,default=None,help="G-Drive link to cached feature data.")
